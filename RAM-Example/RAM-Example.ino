...
unsigned long programTimer;  //timer used in loop() function
...
void loop() {
  ...
  if(millis()-programTimer > 5000){  //write something new every 5 seconds
    checkCorrect();
    randomWrite();
    programTimer = millis();
  }
...
void randomWrite(){
  Serial.print("Now writing new values: ");
  
  for(int i = 0; i < bitNumber; i++){
    int newValue = random(0, 2);
    writeValues[i] = newValue;  //change the array with the values to write
    Serial.print(newValue);
  }
  bitLeftToWrite = bitNumber;  //abilitate writing
  Serial.println("");
}
...
void checkCorrect(){
  ...
  if(canRead){
    for(int i = 0; i < bitNumber; i++){  //check if read bit are the one that were written
        int bitValueRead = bits[i].readBit();
        if(bitValueRead != writeValues[i]){
          correct = false;
          break;
      }
